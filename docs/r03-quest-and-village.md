...
- **당시 테스트 환경 제약:** 세션별 계정 때문에 브라우저 처치 진행 확인이 보류됐었다. 후속 `ac70d3f`에서는 UUID 인증 fixture로 실제 수락→사냥→보상→판매→구매/장착→재접속을 자산 주입 없이 검증했다. [성장 E2E 기록](r07-regions-bosses.md). 실제 KAD gateway SSO 검증과는 구분한다.
