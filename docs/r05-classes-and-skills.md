...
## 9. 후속 자동 검증 보강 (2026-09-22)

실제 로컬 서버 접속을 사용하는 통합 테스트 2개를 추가했다. 초기 직업 선택 패널의 동기화, 2단계 확정, 전사 스킬 요청과 MP·쿨다운 반영, 광장 화염구의 서버 거절 표시를 확인한다. 기존 7개 화이트박스 테스트가 지나지 않던 WorldScene·RoomConnection 연결을 일부 검증하지만, SSO 재접속 복원과 전체 직업 플레이·육안 확인은 여전히 남는다.

E2E TypeScript 오류 7건도 해소했다. 관련 브라우저 테스트 11개와 E2E·workspace typecheck, build가 통과했다. 별도 모델 리뷰 범위는 타입 수정 두 파일이며 지적 사항은 없었다. 상세 명령·결과·한계는 [구현·검증 기록](r05-classes-and-skills.md)에 남겼다.

## R05 실제 접속 통합 테스트 보강

> 원본: `r05-classes-and-skills.md` (날짜별 문서 단일화로 이 절에 통합, 2026-09-23)
