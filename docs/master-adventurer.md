...
기준일: 2026-09-16. `R01`은 **IN PROGRESS**다. skin 0에 4방향 대기·걷기 master 후보를 등록했다. native 48px 규격과 비례는 사용자 시각 승인 전이며, 60프레임 master·24종 스킨 완성을 뜻하지 않는다.
...
| 항목 | 현재 값 |
|---|---|
| runtime texture | `/sprites/master-adventurer.png` |
| 원화·표시 크기 | 모두 48×48px, atlas 192×192px |
| 행 순서 | Down / Left / Right / Up |
| 열 순서 | idle / left-step / passing / right-step |
| 발 기준점 | 셀 내부 `(24, 46)` |
| 대기 | column 0, 반복 없음 |
| 걷기 | columns `[1, 2, 3, 2]`, 프레임당 62.5ms, 반복 |
| 미제작 동작 | attack / cast / hit / death는 idle fallback |

manifest의 `format: "native60"`은 기존 계약 식별자다. 이 asset의 해상도나 제작 프레임 수를 나타내지 않는다. 현재 atlas는 16개 셀이며 전용 공격·시전·피격·사망 원화는 없다. native 48px은 현행 표시 크기에서 출발한 후보이며 당시 게임의 원본 규격이라는 주장이 아니다.

## 원본과 재생성

원본은 [pixels.json](../tools/art-source/master-adventurer/pixels.json)의 palette와 프레임별 pixel grid다. [prepare-master-avatar.mjs](../tools/prepare-master-avatar.mjs)가 이를 native pixel 그대로 atlas에 배치한다. 생성 이미지 후보를 축소해 runtime atlas로 쓰는 방식이 아니다.
